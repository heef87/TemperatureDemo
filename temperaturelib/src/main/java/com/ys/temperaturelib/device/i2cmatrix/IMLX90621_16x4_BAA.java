...
public class IMLX90621_16x4_BAA extends IMatrixThermometer implements TemperatureParser<float[]> {
    public static final String MODE_NAME = "MLX90621-16*4-BAA(矩阵)";
    public static final int MATRIX_COUT_X = 16; //温度矩阵横坐标总数量
    public static final int MATRIX_COUT_Y = 4; //温度矩阵横坐标总数量
    Mlx90621 mMlx90621;

    public IMLX90621_16x4_BAA() {
    ...
    public float check(float value, TemperatureEntity entity) {
        ...
        if (mFloats.size() == 6) {
            ...
            if (entity.ta < 10) {
                tt += 3.5f;
                tt1 += 3.5f;
            } else if (entity.ta >= 10 && entity.ta < 20) {
                tt += 1f;
                tt1 += 1f;
            }
