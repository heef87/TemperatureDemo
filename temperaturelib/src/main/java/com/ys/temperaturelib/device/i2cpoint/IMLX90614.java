...
public class IMLX90614 extends IPointThermometer {
    ...
    public IMLX90614() {
        super(getPath());
        setMeasureParm(new MeasureParm(MODE_NAME, 0, 100, 0, 0));
...
//        entities[4] = entity4;
    ...
    public void setTakeTempEntity(TakeTempEntity takeTempEntity) {
        int distances = takeTempEntity.getDistances();
        if (distances >= 1 && distances <= 8) {
            takeTempEntity.setTakeTemperature(0.3f);
            ...
            takeTempEntity.setTakeTemperature(6f);
        }
        super.setTakeTempEntity(takeTempEntity);
    }
