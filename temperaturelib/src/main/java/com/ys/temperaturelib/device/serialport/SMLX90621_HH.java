...
public class SMLX90621_HH extends ProductImp implements TemperatureParser<byte[]> {
    public static final String DEFAULT_MODE_NAME = "MLX90621-HH(矩阵)"; //型号
    ...
    static final byte[] ORDER_DATA_OUTPUT = new byte[]{(byte) 0xA5, 0x35, 0x01, (byte) 0xDB}; //查询输出数据指令
...
//            getStorager().add(tempCount + ":" + floats + " t:" + tt);
    ...
    public TemperatureEntity parse(byte[] data) {
        ...
        if ((data[0] & 0xFF) == 0x5a && (data[1] & 0xFF) == 0x5a) {
            TemperatureEntity entity = new TemperatureEntity();
            List<Float> temps = new ArrayList<>();
            entity.min = entity.max = ((data[5] & 0xFF) * 256 + (data[4] & 0xFF)) / 100.0f;
            for (int i = 4; i < 131; i = i + 2) {
                int sum = (data[i + 1] & 0xFF) * 256 + (data[i] & 0xFF);
                float temp = sum / 100f;
                Log.e("heef","temp:" + temp);
                if (temp < entity.min) entity.min = temp;
                if (temp > entity.max) entity.max = temp;
                temps.add(temp);
            }
            entity.tempList = temps;
            Log.e("heef","entity.max11:" + entity.max);
            entity.temperatue = check(entity.max, null);
            Log.e("heef","entity.temperatue:" + entity.temperatue);
            return entity;
        }
        return null;
    }
